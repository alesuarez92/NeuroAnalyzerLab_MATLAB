...
classdef MUAAnalysisApp < handle
    ...
    methods
        ...
        %% buildUI - ControlPanel, axes, defined footer (no overlap)
        function buildUI(app)
            T = UITheme;
            app.UIFig = figure('Name', 'MUA Analysis', ...
                'Position', [100 100 820 640], 'Resize', 'on', ...
                'Color', T.bgGray);

            % --- Footer panel (defined section) ---
            ...
            % --- Controls panel (top third, clear boundaries) ---
            app.ControlPanel = uipanel(app.UIFig, 'Title', 'Controls', ...
                'Units', 'normalized', 'Position', [0.01 0.65 0.98 0.33], ...
            ...
            app.LoadBtn = uicontrol(app.ControlPanel, 'Style', 'pushbutton', ...
                ...
                'Position', [0.02 0.72 0.18 0.24], 'FontSize', 10, ...
                'Callback', @(~,~)app.loadData());
            app.StatusLabel = uicontrol(app.ControlPanel, 'Style', 'text', ...
                'String', '', 'Units', 'normalized', ...
                'Position', [0.02 0.52 0.48 0.18], 'FontSize', 10, ...
            ...
            uicontrol(app.ControlPanel, 'Style', 'text', 'String', 'Clusters:', ...
                'Units', 'normalized', 'Position', [0.50 0.58 0.10 0.10], 'FontSize', 10, ...
                'FontWeight', 'bold', 'HorizontalAlignment', 'left');
            app.SpikeRateButton = uicontrol(app.ControlPanel, 'Style', 'pushbutton', 'String', 'Plot Spike Rate', ...
                'Units', 'normalized', 'Position', [0.50 0.36 0.18 0.14], 'FontSize', 10, ...
                'Callback', @(src, event) app.plotSpikeRateOverTime());
            uicontrol(app.ControlPanel, 'Style', 'pushbutton', 'String', 'Check Alignment', ...
                'Units', 'normalized', 'Position', [0.50 0.52 0.18 0.14], 'FontSize', 10, ...
                'Callback', @(~,~)app.plotAlignmentDiagnostics());

            % --- Clusters listbox and buttons (in main figure, between panel and axes) ---
            app.ClusterSelectMenu = uicontrol(app.UIFig, 'Style', 'listbox', ...
                'Units', 'normalized', 'Position', [0.30 0.58 0.14 0.14], ...
                'Max', 2, 'Min', 0, 'String', {}, ...
                'TooltipString', 'Use Ctrl/Shift to select multiple clusters', ...
                'Callback', @(src, ~)app.updateClusterScatter());
            app.SelectAllBtn = uicontrol(app.UIFig, 'Style', 'pushbutton', 'String', 'Select All', ...
                'Units', 'normalized', 'Position', [0.30 0.54 0.06 0.04], ...
                'Callback', @(~,~)selectAllClusters(app));
            app.ClearBtn = uicontrol(app.UIFig, 'Style', 'pushbutton', 'String', 'Clear', ...
                'Units', 'normalized', 'Position', [0.37 0.54 0.06 0.04], ...
                'Callback', @(~,~)clearClusterSelection(app));

            % --- Axes (above footer, below controls) ---
            app.AxStim = axes('Parent', app.UIFig, 'Units', 'normalized', ...
                'Position', [0.08 0.44 0.84 0.18]);
